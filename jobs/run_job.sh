...
export IS_DEBUG_JOB
set +e
/usr/local/src/cinder-ci-2016/jobs/$JOB_TYPE/initialize.sh 2>&1
result_init=$?
...
if [ $result_init -eq 0 ]; then
    ...
    if [[ ! -z "$RUN_TESTS" ]] && [[ "$RUN_TESTS" == "no" ]]; then
        ...
        /usr/local/src/cinder-ci-2016/jobs/run_tests.sh 2>&1
        result_tempest=$?
...
jen_date=$(date +%d/%m/%Y-%H:%M)
/usr/local/src/cinder-ci-2016/jobs/collect_logs.sh 2>&1
result_collect=$?
